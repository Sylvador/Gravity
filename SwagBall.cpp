...
void SwagBall::initShape(const sf::RenderWindow& window)
{
	...
	{
	case DEFAULT:
		color = sf::Color(rand() % 255 + 1, rand() % 255 + 1, rand() % 255 + 1, rand() % 255 + 1);
		break;
	case DAMAGING:
		color = sf::Color::Red;
	...
	if (this->shape.getPosition().y + this->shape.getGlobalBounds().height >= window.getSize().y)
		this->shape.setPosition(this->shape.getPosition().x, window.getSize().y - this->shape.getGlobalBounds().height);
}
...
const float SwagBall::getSize() const
{
